...
# Steady state error "viewed" from the output
essr = array (0, c(N,length(da)))
# Steady state error "viewed" from the reporter
essp = array (0, c(N,length(da)))
# Steady state of output
ssy0 = array (0, c(N,length(da)))
# Steady state of reporter
ssp0 = array (0, c(N,length(da)))
...
anifun = function (s) 
{
	...
	# Add region of essp < 0.05
	.filled.contour(log10(p1seq), log10(p2seq), abs(essp[,,s,1]), levels = c(-1, 0.05), col = c('khaki2', 'white'))
	# Add region of essr < 0.05
	.filled.contour(log10(p1seq), log10(p2seq), abs(essr[,,s,1]), levels = c(-1, 0.05), col = c('lightgreen', 'white'))
	# Calculate levels for contours of essrp
	levs = c(-1, 10^(seq(min(log10(ssp0[,,s,1])), max(log10(ssp0[,,s,1])), length.out=10)))
	levs = signif (levs, 2)
	# Add contour plots of ssp0
	contour (log10(p1seq), log10(p2seq), ssp0[,,s,1], levels = levs, labcex = 1, add=T)
}

...
anifun = function (s) 
{
	...
	.filled.contour(log10(p1seq), log10(p2seq), essr[,,s], levels = c(0, 0.1), col = c('khaki2', 'white'))
	# Add region of essr < 0.05
	.filled.contour(log10(p1seq), log10(p2seq), essr[,,s], levels = c(0, 0.05), col = c('lightgreen', 'white'))
	# Calculate levels for contours of ssy0
	levs = 10^(seq( log10 (min(ssy0[,,s])), log10(max(ssy0[,,s])), length.out = 20))
	levs = signif (levs, 3)
	# Add contour plots of ssy0
	contour(log10(p1seq), log10(p2seq), ssy0[,,s], levels = levs, labcex = 1, add=T)
}
